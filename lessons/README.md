...
4. **Project IDs go last / 工程 ID 后置**  
   `FR/DP/RMD/T/PFR/PDP` are traceability metadata, not beginner vocabulary. Put them near the end of the Notebook.
...
9. **Declarative diagrams over ASCII / 用字符描述流程图而非 ASCII 字符画**  
   Conceptual diagrams use declarative diagram code, primarily Mermaid, rather than hand-aligned ASCII art.

10. **Executable homework contracts / 可执行作业契约**  
    For suitable programming exercises, provide a fixed function signature, a small TODO region, public pytest checks, and a Human Check. Automated checks verify executable behavior; Human Checks verify that the learner can explain the rule. Homework checks are explicit `check_lessonXX.py` files so unfinished coursework does not pollute normal project tests.
...
- [中文作业说明](../exercises/README.zh-CN.md)
- [English exercise guide](../exercises/README.md)

## First learning block / 第一组课程

| ID | 中文 | English | Homework / 作业 | Primary concept / 主要概念 | Engineering slice |
|---|---|---|---|---|---|
| LSN-001 | [从膜电位到一个最小计算神经元](zh/01_membrane_to_lif.ipynb) | [From membrane potential to a minimal computational neuron](en/01_membrane_to_lif.ipynb) | [中文检查说明](zh/01_membrane_to_lif_homework.md) / [English](en/01_membrane_to_lif_homework.md) | purposeful modeling; LIF | RMD-001 |
