...
- Current Python exercise infrastructure result: **128 passed, 2 skipped**. The two skips remain the real Lesson 13 synthesis checks in the Python job where Yosys is absent; the RTL workflow executes that path for real.
...
That places 3 lessons × 2 languages = 6 personal-workbook paths under continuous CI. The current Python exercise infrastructure result is **128 passed, 2 skipped**. The two skips remain the real Lesson 13 synthesis checks in the Python job without Yosys; the RTL workflow executes that path for real.
