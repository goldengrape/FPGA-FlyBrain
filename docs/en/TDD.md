...
- **T-HW-005 PS/Linux first boot**: use the Stage-2 authoring image `iot-limerick-kria-classic-server-2404-classic-24.04-x07-20250423.img.xz` (Ubuntu Server 24.04 LTS for Kria K26), record its locally computed SHA-256, flash it to microSD, and boot KV260 through J4 UART at 115200 8N1 with no flow control. Retain the full boot transcript plus `uname -a`, `/etc/os-release`, `/proc/device-tree/model`, `xmutil boardid`, and `xmutil bootfw_status`. Formal image-hash PASS is blocked while the course manifest's `expected_sha256` remains null; do not invent or silently substitute a hash.
- **T-HW-006 Host↔PL loopback**: after LAB-HW-05 reaches the required runtime-host state, build/program the frozen loopback PL design and use PS/Linux Python MMIO over `/dev/mem` to access dual-channel AXI GPIO at `0xA0010000`; host writes Channel-1 `GPIO_DATA` at `+0x0`, PL computes `(value + 1) mod 2^32`, and host reads Channel-2 `GPIO2_DATA` at `+0x8`. The self-checking script must exercise the fixed vector set, log write/expected/read triples, and distinguish pre-MMIO Linux/permission/policy/transport failures from value mismatches. This `/dev/mem` path remains an authoring-candidate transport until a real Ubuntu 24.04 dry run passes; a policy block is evidence to revise the transport, not permission to weaken system security.
