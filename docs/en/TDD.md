...
- bilingual-ID consistency check: **partially automated**; Exercise Notebooks and LAB-HW-00~05 enforce bilingual cell-structure/ID consistency
- KV260 Physical Lab contract checks: **implemented for LAB-HW-00~05**; CI checks bilingual Notebook structure, frozen XDC/board-helper contracts, LAB-HW-03/04 open-source RTL behavior, Tcl helper paths where they can be exercised without Vivado, and LAB-HW-05 image-identity/runtime-helper contracts. It does not claim physical-board verification or a real Vivado full build.
