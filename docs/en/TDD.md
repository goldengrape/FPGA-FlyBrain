...
- **T-HW-005 PS/Linux first boot**: the course-frozen starter Linux image is version/checksum verified and written to microSD; KV260 boots through the course-specified UART console, produces a boot log, and reaches a shell; the learner distinguishes the development host from the runtime host/PS. During authoring, a locally computed SHA-256 may be recorded while the expected checksum remains unset, but that state is `RECORDED_UNVERIFIED` and cannot satisfy physical T-HW-005 PASS. To avoid a logical dead-end during authoring, complete real boot transcript, shell, board/OS/kernel identity, and clean-shutdown evidence may satisfy the LAB-HW-05 **learning progression gate** into LAB-HW-06; formal T-HW-005 acceptance remains blocked until the expected image hash is frozen.
- **T-HW-006 Host↔PL loopback**: after the LAB-HW-05 learning progression gate is satisfied (formal T-HW-005 acceptance may still be blocked during authoring while the expected image hash is unfrozen), the frozen teaching path is PS/Linux Python → fixed `/dev/mem` MMIO → PS `M_AXI_HPM0_FPD` → SmartConnect → dual-channel AXI GPIO at `0xA0010000`. Channel 1 `GPIO_DATA` (`+0x0`) stores the 32-bit host write; `kv260_loopback_transform` computes `(write + 1) mod 2^32`; Channel 2 `GPIO2_DATA` (`+0x8`) exposes the result. The self-checker runs two rounds of the frozen vectors, retains every write/expected/read triple, and must end in `STATUS=PASS`. Missing device, missing root privilege, OS policy/mmap failure, and PL value mismatch are distinct failure classes. A policy failure must not be bypassed by weakening system security.
...
- KV260 Physical Lab contract checks: **implemented for LAB-HW-00~10**; CI checks bilingual Notebook structure, frozen XDC/board-helper contracts, LAB-HW-03/04/06/07/08 open-source RTL behavior, Tcl helper contracts where they can be exercised without Vivado, LAB-HW-05 image/boot helpers, LAB-HW-06 loopback dry-run, LAB-HW-07 BRAM-state checker, LAB-HW-08 fixture/reference/differential checker, LAB-HW-09 DDR-integrity dry-run/corruption gate, and LAB-HW-10 u-dma-buf prerequisite dry-run, deterministic AXI-CDMA workload dry-run, plus integrity and measurement-instability failure gates. CI does not claim a real Vivado LAB-HW-08/10 build or real-KV260 T-HW-007~010 physical PASS.
