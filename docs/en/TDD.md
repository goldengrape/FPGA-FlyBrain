...
- KV260 Physical Lab contract checks: **implemented for LAB-HW-00~09**; CI checks bilingual Notebook structure, frozen XDC/board-helper contracts, LAB-HW-03/04/06/07 open-source RTL behavior, Tcl helper contracts where they can be exercised without Vivado, LAB-HW-05 image/boot helpers, LAB-HW-06 loopback dry-run, and LAB-HW-07 BRAM-state host self-check behavior. CI checks that the LAB-HW-07 Vivado script contains a non-zero block-RAM resource oracle, but does not claim a real Vivado LAB-HW-07 build or real-board T-HW-007 PASS.
