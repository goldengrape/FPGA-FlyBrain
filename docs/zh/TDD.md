...
- **T-HW-005 PS/Linux first boot**：使用第二阶段 authoring image `iot-limerick-kria-classic-server-2404-classic-24.04-x07-20250423.img.xz`（Kria K26 的 Ubuntu Server 24.04 LTS），记录实际下载 archive 的本地 SHA-256，写入 microSD，并通过 J4 UART 以 115200 8N1、no flow control 启动 KV260。保留完整 boot transcript，以及 `uname -a`、`/etc/os-release`、`/proc/device-tree/model`、`xmutil boardid`、`xmutil bootfw_status`。只要 course manifest 的 `expected_sha256` 仍为 null，就不能宣称 formal image-hash PASS；不得伪造或偷偷替换 hash。
- **T-HW-006 Host↔PL loopback**：LAB-HW-05 达到要求的 runtime-host 状态后，build/program 冻结的 loopback PL design，并由 PS/Linux Python 通过 `/dev/mem` MMIO 访问 `0xA0010000` 的 dual-channel AXI GPIO；host 写 Channel-1 `GPIO_DATA`（`+0x0`），PL 计算 `(value + 1) mod 2^32`，host 从 Channel-2 `GPIO2_DATA`（`+0x8`）读回。self-checking script 必须执行固定 vector、记录 write/expected/read triple，并把第一次 MMIO 前的 Linux/permission/policy/transport failure 与 value mismatch 分开。真实 Ubuntu 24.04 dry run 通过前，`/dev/mem` 仍是 authoring-candidate transport；若系统策略阻止它，应记录并修订 transport，而不是降低系统安全策略。
