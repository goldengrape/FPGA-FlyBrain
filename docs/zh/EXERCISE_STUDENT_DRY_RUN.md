...
- Python exercise infrastructure 当前总结果：**128 passed, 2 skipped**。两个 skip 仍是没有 Yosys 的 Python job 中 Lesson 13 中英文真实 synthesis；RTL workflow 已实际覆盖该路径。
...
因此 3 课 × 2 语言共 6 条个人作业路径进入持续 CI。当前 Python exercise infrastructure 结果为 **128 passed, 2 skipped**；两个 skip 仍是 Python job 中没有 Yosys 时的 Lesson 13 真实综合检查，RTL workflow 会实际执行该路径。
